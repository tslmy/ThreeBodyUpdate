function [communications] = Add2communication(C1,C2,civilizations,communications,systems)
[d,S1,S2] = DC2C(C1,C2,systems)
if and(and(~isempty(find(systems(:,5)==C1)),~isempty(find(systems(:,5)==C2))),or(civilizations(find(civilizations(:,1)==C1),17)>=d,civilizations(find(civilizations(:,1)==C2),17)>=d))
    add(1,1)=C1
    add(1,2)=C2
    add(1,3)=d
    ...
    add(1,8)=0.2*exp(-(log(add(1,4))-log(add(1,5))).^2)
    add(1,9)=0.1
